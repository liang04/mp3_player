...
void i2c_device_add(SSD1306_t *dev, i2c_port_t i2c_num, int16_t reset,
  ...
  i2c_master_dev_handle_t i2c_dev_handle;
  ESP_ERROR_CHECK(i2c_master_bus_add_device(dev->_i2c_bus_handle, &dev_cfg,
                                            &i2c_dev_handle));
...
void i2c_init(SSD1306_t *dev, int width, int height) {
  ...
  esp_err_t res;
  res = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, out_index,
                            I2C_TICKS_TO_WAIT);
  if (res == ESP_OK) {
...
void i2c_display_image(SSD1306_t *dev, int page, int seg, const uint8_t *images,
  ...
  res = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, width + 1,
                            I2C_TICKS_TO_WAIT);
  if (res != ESP_OK)
    ESP_LOGE(TAG, "Could not write to device [0x%02x at %d]: %d (%s)",
             dev->_address, dev->_i2c_num, res, esp_err_to_name(res));
...
void i2c_hardware_scroll(SSD1306_t *dev, ssd1306_scroll_type_t scroll) {
  ...
  esp_err_t res = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, out_index,
                                      I2C_TICKS_TO_WAIT);
  if (res != ESP_OK)
    ESP_LOGE(TAG, "Could not write to device [0x%02x at %d]: %d (%s)",
             dev->_address, dev->_i2c_num, res, esp_err_to_name(res));
